import { User, Booking, Course, Service, Trip, Gallery, sequelize } from '../models/index.js';
...
export const getDashboardStats = tryCatch(async (req, res) => {
  const [
    ...
    Gallery.count(),
    Booking.findAll({
      include: [{ model: User, attributes: ['username', 'fullName'] }, { model: Course, attributes: ['title'] }],
...
export const getAnalytics = tryCatch(async (req, res) => {
  ...
  res.json({
    ...
    revenue: {
      total: totalRevenue,
      average: revenueByPeriod.length > 0 ? totalRevenue / revenueByPeriod.length : 0
    },
    topCourses,
    userRoles
  });
...
export const generateReport = tryCatch(async (req, res) => {
  ...
  switch (type) {
    ...
    default:
      return next(new AppError('Invalid report type. Use: bookings, users, or revenue', 400));
  }
...
export const createStaff = tryCatch(async (req, res, next) => {
  ...
  if (!username || !password || !email || !fullName || !role) {
    return next(new AppError('Username, password, email, fullName, and role are required', 400));
  }
