...
import { useCreateRentalMutation } from "../../../store/rental"
import {
  FiAlertTriangle,
  FiCalendar,
  FiDollarSign,
...
export const NewRentalSchema = zod.object({
...
export type NewRentalSchemaType = zod.infer<typeof NewRentalSchema>
...
export function RentalNewCreateForm() {
  ...
  const defaultValues: NewRentalSchemaType = {
    title: "",
    description: "",
    duration: "",
    location: "",
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      // Send to backend API
      await createRental(formData).unwrap()
      ...
      setError("root", {
        message: error.data?.message || "Failed to create rental listing",
      })
    } finally {
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="pt-4 flex justify-end">
          <button
            ...
            disabled={isSubmitting}
            className="mr-4 px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg shadow-sm transition-colors disabled:opacity-70"
          >
            Cancel
          </button>
          <button
