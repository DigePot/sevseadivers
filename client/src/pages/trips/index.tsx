...
export default function Page() {
  ...
  const destinationOptions = useMemo(() => {
    ...
    const unique = Array.from(new Set(allTrips.map(t => t.destination))).sort()
    return ["All Destinations", ...unique]
  }, [allTrips])
  const activityOptions = useMemo(() => {
    if (!allTrips) return ["All Activities"]
    const unique = Array.from(new Set(allTrips.map(t => t.activityType))).sort()
  ...
  const filteredTrips = useMemo(() => {
    ...
    return allTrips.filter(trip => {
      const matchDestination = destination === "All Destinations" || trip.destination === destination
      const matchActivity = activity === "All Activities" || trip.activityType === activity
  ...
  const paginatedTrips = filteredTrips.slice((page-1)*tripsPerPage, page*tripsPerPage)

  // Reset to page 1 when filters change
  React.useEffect(() => { setPage(1) }, [destination, activity, date])
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: 32 }}>
        <h1 style={{ fontSize: 36, fontWeight: 700, marginBottom: 32 }}>Explore Our Diving and Snorkeling Trips</h1>
        <TripFilters
